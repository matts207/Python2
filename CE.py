...
def one_set_time(api, key, points_with_times, points_without_times, origin):
    ...
    if waypoint_total_time < time_left:
        ...
        waypoints = ''
        return routing(api, key, set_time_address, origin, waypoints)
    else:
        combos = []
        ...
        if not best_choice:
            ...
            w = ''
            return routing(api, key, origin, set_time_address, w) + routing(api, key, set_time_address, origin, wp)
        else:
            ...
            return routing(api, key, origin, set_time_address, waypoints) + routing(api, key, set_time_address, origin, wps_after)
...
def setting_route(api, key, json_file):
    ...
    if len(points_with_times) == 0:
        waypoints = '|'.join('{}'.format(address) for address in points_without_times.keys())
        return routing(api, key, origin, origin, waypoints)
    else:
        if len(points_with_times) == 1:
            return one_set_time(api, key, points_with_times, points_without_times, origin)
